...
class Rating(login_database.Model):
    """
    mapping table with userid fk with item fk for rating
    """
    ...
    rating = login_database.Column(login_database.String,nullable=True)

    user = login_database.relationship('User', foreign_keys='Rating.user_id')
    item = login_database.relationship('Item', foreign_keys='Rating.item_id')
