...
def load_dataset():
    ...
    filepath1 = os.path.realpath('./database/datasets/movielens_small/ratings.csv')
    ratings_dataset= Dataset.load_from_file(filepath1, reader=reader)
    
    # link movie movie's name with its ID 
    movieID_to_Name={}
    filepath = os.path.realpath('./database/datasets/movielens_small/movies.csv')
...
for i in range(1,611):
    ...
    for itemID, rating_sum in sorted(candidates.items(), key=itemgetter(1), reverse=True):
        if not itemID in watched:
            ...
            real_moviid= trainset.to_raw_iid(itemID)
            recommendations.append(real_moviid)
            #recommendations.append(getMovieName(trainset.to_raw_iid(itemID)))
            position +=1
            if(position ==10): break # only top 10 movies to recommend
    all_recoms.append(recommendations)

    for rec in recommendations:
        print("Movie:", rec)
    
save_path=os.path.realpath('./database/datasets/movielens_small/recom_knn_ds1.csv')
recsdf = pd.DataFrame(all_recoms)
recsdf.index = np.arange(1, len(recsdf)+1)
recsdf.to_csv(save_path, sep=",")
